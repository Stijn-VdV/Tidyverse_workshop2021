...
Imagine we want to retain all rows in `publishers` for which we have matching information in `movies`. This returns the original `publishers` object, but without *The Matrix* and *The Notebook* (as they are not part of `movies`).
...
In other words, `semi_join()` keeps all rows in the first dataset (`publishers`) that *have* matching rows (based on the common variable `movie`) in the second dataset (`movies`), but no columns are added (e.g. `publishers$studio`). Conversely, `anti_join()` does the opposite.
...
Let's try and pivot this to a long table using `pivot_longer()`.
